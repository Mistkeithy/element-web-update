...
#
# https://github.com/Mistkeithy/element-web-update
#
###################################################################
...
DIRECTORY_INSTALL=/www/wwwroot/matrix.icu.su
...
VERSION_LATEST=`curl -sL $VERSION_URL | jq -r '.name' | sed s/v//` || { echo "Error checking last Element version!"; exit 1; }
...
if ( [ "$VERSION_INSTALLED" != "$VERSION_LATEST" ] ); then
  ...
  DL_URL=`curl -sL $VERSION_URL | jq -r '.assets[0].browser_download_url'`
